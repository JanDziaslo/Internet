...
$stmt -> execute();
$zesp = $stmt -> fetch(PDO::FETCH_ASSOC);
...
<br>
<h2 class="text-center">Edytuj Zespół</h2>
    ...
    <form action="edytuj_zesp.php?id=<?php echo $id_url;?>" method="post" novalidate>
        ...
        elseif ($zesp === false)
        {
            echo '<br><div class="alert alert-danger d-flex align-items-center" role="alert">
                    <svg class="bi flex-shrink-0 me-2" width="16" height="16" fill="currentColor" role="img" aria-label="Danger:"><use xlink:href="#exclamation-triangle-fill"/></svg>
                    <div>
                    Zespół o podanym ID nie istnieje!
                    </div>
                    </div>';
            exit();
        }
        ...
        <div class="d-flex align-items-center">
            <button type="submit" class="btn btn-primary">Edytuj zespół</button>
